...
function OpenCloakroomMenu()
	...
	ESX.UI.Menu.Open(
		'default', GetCurrentResourceName(), 'cloakroom',
		{
			...
			align    = 'top-left',
			elements = {
				{label = _U('nightclub_clothes_civil'), value = 'citizen_wear'},
				{label = _U('nightclub_clothes_nightclub'), value = 'nightclub_wear'},
			},
		},
		function(data, menu)
			...
			if data.current.value == 'nightclub_wear' then
				ESX.TriggerServerCallback('esx_skin:getPlayerSkin', function(skin, jobSkin)
					if skin.sex == 0 then
...
function OpenNightclubActionsMenu()
...
function OpenVehicleSpawnerMenu()
	...
	if Config.EnableSocietyOwnedVehicles then
		...
		ESX.TriggerServerCallback('esx_society:getVehiclesInGarage', function(vehicles)
			...
			ESX.UI.Menu.Open(
				...
				function(data, menu)
					...
					ESX.Game.SpawnVehicle(vehicleProps.model, Config.Zones.VehicleSpawnPoint.Pos, 357.739, function(vehicle)
		...
		local elements = {
			{label = 'Burrito (Flyers)',  value = 'burrito3'},
			{label = 'Véhicule Patron',  value = 'ztype'},
		}
		...
		ESX.UI.Menu.Open(
			...
			function(data, menu)
				...
				ESX.Game.SpawnVehicle(model, Config.Zones.VehicleSpawnPoint.Pos, 357.739, function(vehicle)
					local playerPed = GetPlayerPed(-1)
					TaskWarpPedIntoVehicle(playerPed,  vehicle,  -1)
...
function OpenMobileNightclubActionsMenu()

	ESX.UI.Menu.CloseAll()

	ESX.UI.Menu.Open(
		'default', GetCurrentResourceName(), 'mobile_nightclub_actions',
		{
			css = 'nightclub',
			align    = 'top-left',
			elements = {
						...
						if amount == nil or amount <= 0 then
							...
							if closestPlayer == -1 or closestDistance > 3.0 then
								...
								Citizen.CreateThread(function()
									...
									TriggerServerEvent('esx_billing:sendBill', GetPlayerServerId(closestPlayer), 'society_nightclub', 'Nightclub', amount)
								end)
							end
						end
					end,
...
function OpenShopMenu(zone)
    ...
    for i=1, #Config.Zones[zone].Items, 1 do
        ...
        table.insert(elements, {
            label     = item.label .. ' <span style="color:gray;">$' .. item.price .. ' </span>',
...
AddEventHandler('esx_nightclub:hasEnteredMarker', function(zone)
	if zone == 'AbsintheFarm' and PlayerData.job ~= nil and PlayerData.job.name == 'nightclub'  then
		CurrentAction     = 'raisin_harvest'
		CurrentActionMsg  = _U('press_collect')
		CurrentActionData = {zone= zone}
	end
		
	if zone == 'TraitementMdlm' and PlayerData.job ~= nil and PlayerData.job.name == 'nightclub'  then
		CurrentAction     = 'mdlm_traitement'
		CurrentActionMsg  = _U('press_traitement')
		CurrentActionData = {zone= zone}
	end		
		
	if zone == 'SellFarm' and PlayerData.job ~= nil and PlayerData.job.name == 'nightclub'  then
		CurrentAction     = 'farm_resell'
		CurrentActionMsg  = _U('press_sell')
		CurrentActionData = {zone = zone}
	end

	if zone == 'Shop' and PlayerData.job ~= nil and PlayerData.job.name == 'nightclub' and PlayerData.job.grade_name == 'boss' then
		CurrentAction     = 'menu_shop'
		CurrentActionMsg  = _U('shop_menu')
		CurrentActionData = {zone = zone}
	end

	if zone == 'NightclubActions' and PlayerData.job ~= nil and PlayerData.job.name == 'nightclub' then
		CurrentAction     = 'nightclub_actions_menu'
		CurrentActionMsg  = _U('press_to_open')
		CurrentActionData = {}
	end
  
	if zone == 'VehicleSpawner' and PlayerData.job ~= nil and PlayerData.job.name == 'nightclub' then
		CurrentAction     = 'vehicle_spawner_menu'
		CurrentActionMsg  = _U('spawn_veh')
		CurrentActionData = {}
	end
		
	if zone == 'VehicleDeleter' and PlayerData.job ~= nil and PlayerData.job.name == 'nightclub' then
...
AddEventHandler('esx_nightclub:hasExitedMarker', function(zone)
	ESX.UI.Menu.CloseAll()
	if (zone == 'AbsintheFarm') and PlayerData.job ~= nil and PlayerData.job.name == 'nightclub' then
		TriggerServerEvent('esx_nightclub:stopHarvest')
	end  
	if (zone == 'TraitementMdlm') and PlayerData.job ~= nil and PlayerData.job.name == 'nightclub' then
		TriggerServerEvent('esx_nightclub:stopTransform')
	end
	if (zone == 'SellFarm') and PlayerData.job ~= nil and PlayerData.job.name == 'nightclub' then
		TriggerServerEvent('esx_nightclub:stopSell')
	end
	CurrentAction = nil
end)
...
function blips()
	if publicBlip == false then
		local blip = AddBlipForCoord(-1285.182, -652.152, 26.633)
...
Citizen.CreateThread(function()
	while true do
		...
		if CurrentAction ~= nil then
			...
			if IsControlPressed(0,  Keys['E']) and PlayerData.job ~= nil and PlayerData.job.name == 'nightclub' and (GetGameTimer() - GUI.Time) > 300 then
				if CurrentAction == 'raisin_harvest' then
					TriggerServerEvent('esx_nightclub:startHarvest', CurrentActionData.zone)
				end
				if CurrentAction == 'mdlm_traitement' then
					TriggerServerEvent('esx_nightclub:startTransform', CurrentActionData.zone)
				end
				if CurrentAction == 'farm_resell' then
					TriggerServerEvent('esx_nightclub:startSell', CurrentActionData.zone)
				end
				if CurrentAction == 'menu_shop' then
					OpenShopMenu(CurrentActionData.zone)
				end
				if CurrentAction == 'nightclub_actions_menu' then
					OpenNightclubActionsMenu()
				end
		...
		if IsControlPressed(0,  Keys['F6']) and Config.EnablePlayerManagement and PlayerData.job ~= nil and PlayerData.job.name == 'nightclub' and (GetGameTimer() - GUI.Time) > 150 then
...
Citizen.CreateThread(function()
    ...
    AfterHoursNightclubs.Interior.Name.Set(AfterHoursNightclubs.Interior.Name.palace, true)
